...
def get_date_columns(df: pd.DataFrame) -> list:
    # Exclude ID/label columns, AND keep only numeric columns.
    # This protects the script even if it's accidentally run on a
    # dataset that has extra non-numeric columns (e.g. 'theft_type'
    # added later by the Theft Generator step).
    exclude = ["CONS_NO", "FLAG"]
    ...
    dropped = set(candidate_cols) - set(numeric_cols)
    if dropped:
        print(f"Note: Ignored {len(dropped)} non-numeric column(s) not treated as readings: {sorted(dropped)}\n")
...
if __name__ == "__main__":
    # Task-1 runs on the RAW SGCC dataset (downloaded from Kaggle),
    # to give a baseline BEFORE SMOTE balancing / theft generation.
    DATASET_PATH = r"C:\Ayesha's VS CODES\GridEye Codes\dataset.csv"
